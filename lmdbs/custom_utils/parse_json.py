...
def getJsonDict(json_path,show_dict=False):
    '''
...
def abstract_data_2_onedir(src, dst):
    ...
    for base in bases:
        ...
        if osp.isdir(s):
            ...
            shutil.copyfile(s, osp.join(dst, f'{osp.basename(osp.dirname(s))}_{osp.basename(s)}'))
...
if __name__ == '__main__':
    ...
    dst1 = r'E:\lxd_dataset\Museum\cut_out_all\cuted_data\from_image'
    dst2 = r'E:\lxd_dataset\Museum\cut_out_all\cuted_data\from_video'
    ...
    dst_dir = r'D:\dataset\OCR\ocr_label_0620/2200'
    img_paths_list = get_second_order_img(r'D:\dataset\OCR\ocr_label_0620')

    # np.random.shuffle(img_paths_list)
    # for img_path in tqdm(img_paths_list[:2200]):
    #     shutil.copyfile(img_path,osp.join(dst_dir,osp.basename(img_path)))
    # pprint(img_paths_list[:5])
