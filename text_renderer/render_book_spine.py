...
from lv_tools.task_book_info_gen.book_info_content import BookSpineContent, callnumber_split
...
from text_renderer.utils.types import FontColor
...
class BookSpineRender(Render):
    ...
    * 存储器需要解耦出来
    ...
    def __init__(self, cfg: RenderCfg,
        ...
        '''
            {'0主标题': ['马克思恩格斯书信集'],
             ...
             '第二责任者': []})
        '''
        self._init_corpus(book_spine_content, book_info_template)
        self._corpus_check()
    ...
    def _init_corpus(self, book_spine_content: BookSpineContent, book_info_template: BookInfoTemplate):
        ...
        while True:
            self.corpus.normalized_corpus = book_spine_content.get_items_randomly()
            if not self.corpus.normalized_corpus.get('0主标题'):
                continue
            title_text_num = len(self.corpus.normalized_corpus['0主标题'][0])
            self.book_info_template_path = book_info_template.get_template_path_by_book_name_length(title_text_num)
            break

    def _init_callnumber_corpus(self, book_spine_content: BookSpineContent, book_info_template: BookInfoTemplate):
        '''
        ...
        while True:
            self.corpus.normalized_corpus = book_spine_content.get_items_randomly()
            callnumber_list = callnumber_split(self.corpus.normalized_corpus['7索书号'])
            callnumber_num = len(callnumber_list)
            self.book_info_template_path = book_info_template.get_template_path_by_book_name_length(callnumber_num)
            break
    ...
    def __call__(self, *args, **kwargs):
        ...
        try:
            ...
            bg = self.bg_gener.gen_mimic_spine_bg((bg_h, bg_w))
            text_len_limit = 1.2
            dst_shapes = []
...
class CallnumberRender(BookSpineRender):
    ...
    def _init_corpus(self, book_spine_content: BookSpineContent, book_info_template: BookInfoTemplate):
        ...
        while True:
            ...
            callnumber_num = len(callnumber_list)
            self.book_info_template_path = book_info_template.get_template_path_by_callnumber_part_num(callnumber_num)
            break
